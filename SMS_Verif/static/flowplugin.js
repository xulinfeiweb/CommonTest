...
(function ($) {
	$.fn.flowplugin = function (opt) {
		opt = $.extend({
			jsonDate: [],//json数据
			imgSrcStart: "../static/img/f1.png",//最新流程节点图片
			imgSrcOther: "../static/img/f2.png",//其它流程节点图片
			imgWidth: "20px",//图片宽  设置第一个图片的宽度
			imgHeight: "20px",//图片高  设置第一个图片的高度
			imgClass: "firstImg"//图片宽高样式   （如果传入imgClass 则不需要设置imgHeight,imgHeight）
		}, opt);
		...
		function _makeContent(obj) {
			...
			$.each(data, function (index, item) {
				var v_time = item.createDate;
				var v_status = item.status;
				...
				//左边状态
				var cellLeftHtml = '<p class="cell-size">' + v_status + '</p>';
				...
				//右边内容
				var cellRightHtml = index == 0 ? '<p class="cell-size">' + v_time + '</p><p class="cell-size">' + v_name + '</p>' : '<p class="cell-size">' + v_time + '</p><p class="cell-size">' + v_name + '</p>';
