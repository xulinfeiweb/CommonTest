    ...
    public class PDFInvoice : PDFBase
    {
        ...
        private void Navigation()
        {
            if (!string.IsNullOrWhiteSpace(_topPath))
                AddImage(_topPath, 150, 30, _doc.Left, _doc.Top - 38);
        }
        private void Header()
        {
            ...
            table.AddCell(cell);
            //Row 8
            cell = CreatePdfPCellNoBorder(new Phrase(header.b3.Replace("\n"," "), JointacFont.FontCN(fontSize)), 1, 1, Element.ALIGN_LEFT, Element.ALIGN_MIDDLE);
            table.AddCell(cell);
        ...
        private void BodyLast(List<InvoiceModel> list, bool isLastPage = false)
        {
            ...
            if (_model.Chapter == "true")
            {
                ...
                if (list.Count < ROW)
                {
                    int rows = ROW - list.Count;
                    if (list.Where(p => p.IsFather == 1).ToList().Count > 0)
                        rows--;
                    yheight = rows * LINE_HEIGHT;
                }
        ...
        private void BodyLast2(List<InvoiceModel> list, bool isLastPage = false)
        {
            ...
            table.TotalWidth = _doc.PageSize.Width - 20;
            table.LockedWidth = true;
            table.SetWidths(new float[] { 4, 10, 8, 8, 26, 5, 4, 7, 7, 6, 8, 7 });
            table.HeaderRows = 1;
            PdfPCell cell;
            #region  
        ...
        private void Footer()
        {
            ...
            {
                Chunk chunk1 = new Chunk(footer.footer.Substring(0, 71), JointacFont.FontCN());
                Chunk chunk2 = new Chunk(footer.footer.Substring(71), JointacFont.FontCN());
                Phrase p1 = new Phrase();
                Phrase p2 = new Phrase();
                p1.Add(chunk1);
                p2.Add(chunk2);
                ColumnText.ShowTextAligned(_writer.DirectContent, Element.ALIGN_LEFT, p1, _doc.PageSize.Width / 2 + 95, _doc.Bottom + 15, 0);
                ColumnText.ShowTextAligned(_writer.DirectContent, Element.ALIGN_LEFT, p2, _doc.PageSize.Width / 2 + 95, _doc.Bottom + 5, 0);
            }
            #endregion
        }
    }
