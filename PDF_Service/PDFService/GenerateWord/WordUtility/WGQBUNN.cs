...
namespace Common
{
    ...
    {
        public WGQBUNNUtility(string saveFile)
        {
        ...
        public bool GenerateWord(object tempFile, List<InvoiceModel> list, Dictionary<string, string> dic)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < list.Count; i++)
                {
                    ...
                    InsertCell(1, i + 2, 5, list[i].H2000Index);
                    InsertCell(1, i + 2, 6, list[i].ClearQty.ToString("0.000"));
                    InsertCell(1, i + 2, 7, list[i].NetWeight.ToString("0.000"));
                    InsertCell(1, i + 2, 8, list[i].NetWeight.ToString("0.000"));
                    InsertCell(1, i + 2, 9, list[i].UnitPrice.ToString("0.00"));
                    InsertCell(1, i + 2, 10, (list[i].UnitPrice * list[i].ClearQty).ToString("0.00"));
                    InsertCell(1, i + 2, 11, list[i].CurrencyEN);
                    InsertCell(1, i + 2, 12, list[i].MadeInEN);
                    InsertCell(1, i + 2, 13, list[i].HSCode);
                    for (int j = 0; j < 13; j++)
                    {
                        SetFont_Table(1, i + 2, j + 1, "Arial", 6.5, 0);
                        #region 单元格对齐方式
                        int Align = -1;//默认左对齐
                        //左对齐列
                        //int[] arr1 = { 0, 1, 2, 3, 4 ,10,11,12};
                        //右对齐列
                        int[] arr = { 5, 6, 7, 8, 9 };
                        if (arr.Contains(j))
                            Align = 1;
                ...
                SetFont_Table(1, TotalIndex, 4, "Arial", 6.5, 1);
                SetParagraph_Table(1, TotalIndex, 4, -1, 0);

                InsertCell(1, TotalIndex, 6, sum.ToString("0.000"));
                SetFont_Table(1, TotalIndex, 6, "Arial", 6.5, 1);
                SetParagraph_Table(1, TotalIndex, 6, 1, 0);

                InsertCell(1, TotalIndex, 7, NetWeight.ToString("0.000"));
                SetFont_Table(1, TotalIndex, 7, "Arial", 6.5, 1);
                SetParagraph_Table(1, TotalIndex, 7, 1, 0);

                InsertCell(1, TotalIndex, 8, NetWeight.ToString("0.000"));
                SetFont_Table(1, TotalIndex, 8, "Arial", 6.5, 1);
                SetParagraph_Table(1, TotalIndex, 8, 1, 0);

                InsertCell(1, TotalIndex, 9, "");
                SetFont_Table(1, TotalIndex, 9, "Arial", 6.5, 1);
                SetParagraph_Table(1, TotalIndex, 9, 1, 0);

                InsertCell(1, TotalIndex, 10, Amount.ToString("0.00"));
                SetFont_Table(1, TotalIndex, 10, "Arial", 6.5, 1);
                SetParagraph_Table(1, TotalIndex, 10, 1, 0);

                InsertCell(1, TotalIndex, 11, Currency);
                SetFont_Table(1, TotalIndex, 11, "Arial", 6.5, 1);
                SetParagraph_Table(1, TotalIndex, 11, -1, 0);
                #endregion
                #endregion
