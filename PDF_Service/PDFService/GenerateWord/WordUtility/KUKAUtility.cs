...
namespace Common
{
    ...
    /// KUKA
    /// </summary>
    public class KUKAUtility : WordBase
    {
        public KUKAUtility(string saveFile)
        {
        ...
        /// <param name="list">重复表格的数据</param>
        /// <param name="dic">word中要替换的表达式和表格字段的对应关系</param>
        public bool GenerateWord2(object tempFile, List<InvoiceModel> list, Dictionary<string, string> dic)
        {
            if (!File.Exists(tempFile.ToString()))
            {
                return false;
